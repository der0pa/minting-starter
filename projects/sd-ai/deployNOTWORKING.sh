...
IDENTITY_NAME="mainnet-ed25519-identity"

# NFT collection settings
COLLECTION_ID="sd"
DISPLAY_NAME="diffusion ai"
NAMESPACE="sd.ai"
TOKEN_PREFIX="sd-"
...
if [[ $IC_NETWORK == 'local' ]]; then
  ...
  dfx canister --network $IC_NETWORK create origyn_sale_reference || true

fi

NFT_CANISTER_ID=$(dfx canister --network $IC_NETWORK id origyn_nft_reference)
echo "NFT_CANISTER_ID: $NFT_CANISTER_ID"
SALE_CANISTER_ID=$(dfx canister --network $IC_NETWORK id origyn_sale_reference)
echo "NFT_CANISTER_ID: $SALE_CANISTER_ID"
...
dfx build --network $IC_NETWORK origyn_sale_reference

if [[ $IC_NETWORK == 'ic' ]]; then
  gzip -k ./.dfx/ic/canisters/origyn_sale_reference/origyn_sale_reference.wasm
  dfx canister --network $IC_NETWORK install origyn_sale_reference --mode=reinstall --wasm ./.dfx/ic/canisters/origyn_sale_reference/origyn_sale_reference.wasm.gz --argument "(record {owner = principal \"$ADMIN_PRINCIPAL\"; storage_space = null})"
else
  dfx canister --network $IC_NETWORK install origyn_sale_reference --mode=reinstall --argument "(record {owner = principal \"$ADMIN_PRINCIPAL\"; storage_space = null})"
fi
...
if [[ $IC_NETWORK == 'local' ]]; then
  ...
  dfx canister --network $IC_NETWORK create origyn_sale_reference
  ORIGYN_SALE_REFERENCE_CANISTER_ID=$(dfx canister --network $IC_NETWORK id origyn_sale_preference)

  echo "ORIGYN_SALE_REFERENCE_CANISTER_ID: $ORIGYN_SALE_REFERENCE_CANISTER_ID"

  if [[ $ORIGYN_SALE_REFERENCE_CANISTER_ID == '' ]]; then
  ...
  dfx build --network $IC_NETWORK origyn_sale_preference
  dfx canister --network $IC_NETWORK install origyn_sale_preference --mode=reinstall --argument "(principal \"$ADMIN_PRINCIPAL\")"
